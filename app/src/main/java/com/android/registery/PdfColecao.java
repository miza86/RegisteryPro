    ...
    public Document gerarDocumento(String autor) {
        ...
        try {
            ...
            for (int i = 1; i <= cursor.getCount(); i++) {
                ...
                Image img = Image.getInstance(cursor.getString(cursor.getColumnIndex("path")));
                if (img.getHeight() > img.getWidth())
                    img.scaleAbsolute(180, 240);
                else img.scaleAbsolute(320, 240);
                doc.add(img);
